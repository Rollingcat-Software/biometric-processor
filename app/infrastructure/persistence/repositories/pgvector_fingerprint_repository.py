"""PostgreSQL pgvector repository for fingerprint embeddings.

Follows the same centroid-based storage pattern as PgVectorVoiceRepository.
Each enrollment is stored as an INDIVIDUAL row, and a CENTROID row is
maintained as the running average of all individual embeddings.
...
class PgVectorFingerprintRepository:
    """Repository for fingerprint enrollment embeddings using pgvector.

    Table: fingerprint_enrollments
    Pattern: individual enrollments + centroid average (same as voice/face)
    """
    ...
    def __init__(
        ...
        pool_min_size: int = 2,
        pool_max_size: int = 5,
        embedding_dimension: int = FINGERPRINT_EMBEDDING_DIM,
    ) -> None:
        ...
        logger.info(
            f"Initialized PgVectorFingerprintRepository "
    ...
    async def _ensure_pool(self) -> asyncpg.Pool:
        ...
        if self._pool is None:
            try:
                ...
                logger.info("Fingerprint repository connection pool created")
            except Exception as e:
                logger.error(f"Failed to create fingerprint repo pool: {e}", exc_info=True)
    ...
    async def save(
        ...
        """Save a fingerprint embedding (INDIVIDUAL) and update CENTROID.

        Args:
            user_id: User identifier.
            embedding: 256-dim fingerprint embedding vector.
            quality_score: Quality score (0-1).
            tenant_id: Optional tenant identifier.
            finger_position: Which finger (e.g. RIGHT_INDEX, LEFT_THUMB).
        """
        ...
        try:
            ...
            async with pool.acquire() as conn:
                ...
                await conn.execute(
                    ...
                    INSERT INTO fingerprint_enrollments (
                        user_id, tenant_id, embedding, quality_score,
                        finger_position, enrollment_type
                    ) VALUES ($1::varchar, $2::varchar, $3, $4, $5::varchar, 'INDIVIDUAL'::varchar)
                    """,
                    user_id, tenant_id, embedding_list, quality_score, finger_position,
                )

                # Check if centroid exists
                has_centroid = await conn.fetchval(
                    """
                    SELECT count(*) FROM fingerprint_enrollments
                ...
                if has_centroid == 0:
                    ...
                    await conn.execute(
                        f"""
                        UPDATE fingerprint_enrollments SET
                        ...
                        WHERE fingerprint_enrollments.user_id = $1::varchar
                          AND fingerprint_enrollments.enrollment_type = 'CENTROID'
                        """,
                        user_id,
                    )
            ...
            logger.info(
                f"Fingerprint embedding saved: user_id={user_id}, "
                f"dim={len(embedding)}, finger={finger_position}, "
            ...
            logger.error(f"Failed to save fingerprint embedding: {e}", exc_info=True)
            raise RepositoryError(operation="save", reason=str(e))

    async def find_by_user_id(
        self, user_id: str, tenant_id: Optional[str] = None
    ) -> Optional[np.ndarray]:
        """Find the fingerprint centroid embedding for a user.
        ...
        try:
            ...
            logger.error(f"Failed to find fingerprint embedding: {e}", exc_info=True)
    ...
    async def delete_by_user_id(self, user_id: str) -> bool:
        ...
        try:
            ...
            logger.info(f"Fingerprint enrollments soft-deleted: user_id={user_id}, affected={affected}")
            return affected > 0

        except Exception as e:
            logger.error(f"Failed to delete fingerprint enrollment: {e}", exc_info=True)
            raise RepositoryError(operation="delete", reason=str(e))

    async def find_similar(self, embedding, threshold=0.4, limit=5):
        """Search for similar fingerprint embeddings (1:N identification)."""
        try:
            pool = await self._ensure_pool()
            embedding_list = embedding.tolist()
            ...
            logger.error(f"Fingerprint search failed: {e}", exc_info=True)
